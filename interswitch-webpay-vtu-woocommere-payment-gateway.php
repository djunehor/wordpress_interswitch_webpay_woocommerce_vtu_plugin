<?php
/*
	Plugin Name: Interswitch Webpay WooCommerce Payment Gateway for VTU Portal
	Description: Interswitch Webpay WooCommerce Payment Gateway allows you to accept payment on your Woocommerce store via debit card. on success, user order is processed via third party API
	Version: 4.0.1
...
*/
include_once ('../mobilenig/buy_data.php');
if ( ! defined( 'ABSPATH' ) )
	exit;
...
function tbz_wc_interswitch_webpay_init() {
	...
	class WC_Tbz_Webpay_Gateway extends WC_Payment_Gateway {
		...
		**/
				...
				if( '00' == $response_code ) {
					...
					if(  $amount_paid < $order_total ) {
						...
						foreach ($order->get_items() as $item_id => $item_data) {
							...
							$terms = wp_get_post_terms( $item_data->get_product_id(), 'product_cat', array( 'fields' => 'names' ) );

                            $network = $terms[0];

							$item_total = $item_data->get_total(); // Get the item line total
                            $amount = $item_total + ($item_total * 1.5/100);

							break;
						}


						if(isset($network) && isset($amount)) {
						    if($this->is_valid_number($network, $order_billing_phone)) {
							    $purchaseData = new MobileNig( $order_billing_phone, $network, $amount, $txn_ref );

							    if ( $purchaseData ) {
								    //Add admin order note
								    $order->add_order_note( 'Data purchase via MobileNig<br />Transaction Reference: ' . $txnref . '<br />Payment Reference: ' . $payment_ref );

								    //Add customer order note
								    $order->add_order_note( 'Order Successful.<br />Transaction Reference: ' . $txnref . '<br />Payment Reference: ' . $payment_ref, 1 );
								    ...
								    $message      = 'Purchase of data Failed. Please contact an admin. <br />Transaction Reference: ' . $txnref;
								    $message_type = 'error';
							    ...
							    $message = "Purchase of data Failed. Provided number [$order_billing_phone] doesn't match specified network [$network]. <br />Transaction Reference: " . $txnref;
							    $message_type = 'error';
		...
		public function is_valid_number($network, $phone) {
		    $mtn = ['0803', '0806', '0703', '0706', '0813', '0816'];
		    $glo = ['0805', '0807', '0705', '0815', '0817'];
		    $airtel = ['0802', '0808', '0812'];
		    $mobile9 = ['0809', '0818', '0708'];
		    ...
		    if(in_array($prefix, $mtn))
		        $phoneNetwork = 'MTN';
			if(in_array($prefix, $glo))
				$phoneNetwork = 'GLO';
			if(in_array($prefix, $airtel))
				$phoneNetwork = 'AIRTEL';
			if(in_array($prefix, $mobile9))
				$phoneNetwork = '9MOBILE';

			if($phoneNetwork != $network)
			    return false;
